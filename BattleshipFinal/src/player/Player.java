...
public class Player {
    ...
    private ArrayList<Ship> initShips(){
        ...
        for (ShipType type: ShipType.values()){
            for (int i = 0; i < type.getNumShips(); i++){
                list.add(new Ship(ShipType.toNameShip(type), type.getShipLength()));
            }
        }
        return list;
    }
    ...
    public void addAllShips(){
        if (!isAI) {
            ...
            Position position;
            Direction direction;
            String messageInputPosition = "- Enter the coordinate (eg a1): ";
            String messageInputDirection = "- Enter the direction (h/v): ";
